...
Description
    C++ programming tutorial - guess two numbers - using a function to read data from the terminal
...
//>>>>>>>>>>>>>>>>>>>>>>>>>//
//-  move this to readFromTerminal.H  
//-  replace the code by: vector input = readFromTerminal();

    vector input=readFromTerminal();         //- declare a vector called input
